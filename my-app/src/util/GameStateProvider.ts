...
import { BE_URL, BE_WS_URL } from '../api/api'
...
export const useGameState = () => {
  ...
  const copyState = useCallback((): GameState => {
    ...
    for (let i = 0; i < gameState.board.length; i++) board[i] = gameState.board[i].slice()
  ...
  const moveLetterToPouchFromHand = async (letter: LetterTile) => {
    ...
    if (
      gameStateCopy.playerHands[getCurrentPlayerName()].findIndex(
        (handLetter) => handLetter.id === letter.id
      ) === -1
    )
      return

    const idx = gameStateCopy.playerHands[getCurrentPlayerName()].findIndex(
      (l) => l.id === letter.id
    )
  ...
  const getLetterPosition = (letterTile: LetterTile) => {
    for (let i = 0; i < gameState.board.length; i++)
      for (let j = 0; j < gameState.board.length; j++)
        if (gameState.board[i][j].letterTile?.id === letterTile.id) return { x: i, y: j }
